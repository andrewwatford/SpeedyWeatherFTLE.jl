...
function lyapunov_FTLE(; 
    ...
    default_grid_type=HEALPixGrid, 
    use_default_grid=true,
    ...
    Inputs:
        ...
        default_grid_type: spectral grid type to use if use_default_grid is true
        use_default_grid: if true, uses a default grid with the option of random or climatological velocity field.
        model_type: model type. Currently only one-layer models are supported
        use_climatological: if true, take initial velocity field of simulation. If false, take random initial velocity field
    ...
    if use_default_grid
        ...
        spectral_grid = SpectralGrid(nlayers=1, Grid=default_grid_type)
        nparticles = 4*spectral_grid.npoints
        grid = SpectralGrid(nlayers=1, nparticles=nparticles, Grid=default_grid_type)
    else
        # TODO: Replace this with extracting the grid from the given velocity field
        error("Custom grid type not yet implemented")
    end
    
    # Set up particle advection scheme, model, and simulation
    particle_advection = ParticleAdvection2D(grid, backwards=backwards)
    model = model_type(grid, dynamics=dynamics; particle_advection)
    ...
    elseif use_default_grid
        u = randn(grid.grid)
        v = randn(grid.grid)
        simulation.diagnostic_variables.grid.u_grid .= u
        simulation.diagnostic_variables.grid.v_grid .= v
    else
        # TODO: change this to make use of the velocity field given
        error("Custom velocity field not yet implemented")
    end

    # Add particle tracker to the model
    particle_tracker = ParticleTracker(grid, schedule=Schedule(every=Hour(rint_hours)))
    add!(model, :particle_tracker => particle_tracker)

    ### Perturb initial locations of particles ###
    londs, latds = RingGrids.get_londlatds(grid.grid)
    ...
    FTLE_grid_time = Array{Float64}(undef, grid.npoints, particles_ds.dim["time"]) 
    ...
    return FTLE_grid_time, grid.grid, time_hours
